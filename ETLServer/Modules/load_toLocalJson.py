...
def load_TstatsJsonData(tmp_data, league_id):
    ...
    with open("%s/teams/statistics/%s/%s_%s_Tstats.json" % (directory, now_date, now_date, league_id), "r") as json_file:
        data = json.load(json_file)
    ...
    with open("%s/teams/statistics/%s/%s_%s_Tstats.json" % (directory, now_date, now_date, league_id), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print(league_id)
        print("sucksex")
...
def load_lineUpsJson(tmp_data):
    now_date = datetime.utcnow().date().strftime("%y%m%d")
    directory = os.path.join(os.path.dirname(__file__), "..", 'datas/DataLake/fixtures/lineups')

    with open("%s/%s/%s_lineUps.json" %(directory, now_date, now_date), "r") as json_file:
        data = json.laod(json_file)

    data['data'].append(tmp_data)

    with open("%s/%s/%s_lineUps.json" % (directory, now_date, now_date), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print("sucksex")
...
def load_leagueJson(tmp_data, league_id):
    ...
    with open("%s/%s_%d_leagues.json" % (directory, now_date, league_id), "r") as json_file:
        data = json.load(json_file)
    ...
    with open("%s/%s_%d_leagues.json" % (directory, now_date, league_id), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print("sucksex")
...
def load_psquadJson(tmp_data, team_id):
    ...
    now_date = datetime.utcnow().date().strftime("%y%m%d")
    directory = os.path.join(os.path.dirname(__file__), "..", 'datas', 'DataLake', 'players', 'squad')

    with open("%s/%s_%d_Psquad.json" % (directory, now_date, team_id), "r") as json_file:
        data = json.load(json_file)

    data['data'].append(tmp_data)
    print(tmp_data)

    with open("%s/%s_%d_Psquad.json" % (directory, now_date, team_id), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print("sucksex")
...
def load_pplayerJson(tmp_data, league_id):
    ...
    now_date = datetime.utcnow().date().strftime("%y%m%d")
    directory = os.path.join(os.path.dirname(__file__), "../datas/DataLake/players/players")

    with open("%s/%s_%s_players.json" % (directory, now_date, league_id), "r") as json_file:
        data = json.load(json_file)

    data['data'].append(tmp_data)
    print(tmp_data)

    with open("%s/%s_%s_players.json" % (directory, now_date, league_id), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print("sucksex")
...
def load_ptopscorersJson(tmp_data, league_id):
    ...
    now_date = datetime.utcnow().date().strftime("%y%m%d")
    directory = os.path.join(os.path.dirname(__file__), "..", 'datas', 'DataLake', 'players', 'topscorers')

    with open("%s/%s_%d_Ptopscorers.json" % (directory, now_date, league_id), "r") as json_file:
        data = json.load(json_file)

    data['data'].append(tmp_data)
    print(tmp_data)

    with open("%s/%s_%d_Ptopscorers.json" % (directory, now_date, league_id), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print("sucksex")
...
def load_predictionsJsonData(tmp_data):
    ...
    with open("%s/%s/%s_predictions.json" % (directory, now_date, now_date), "r") as json_file:
        data = json.load(json_file)
    ...
    with open("%s/%s/%s_predictions.json" % (directory, now_date, now_date), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print("sucksex")