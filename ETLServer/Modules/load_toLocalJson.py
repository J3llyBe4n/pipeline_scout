...
def load_h2hJson(tmp_data): #
    ...
    with open("%s/%s_%s_h2h.json" %(directory, now_date), "r") as json_file: #
        data = json.load(json_file)

    data['data'].append(tmp_data)

    with open("%s/%s_%s_h2h.json" %(directory, now_date), "w") as json_file: #
        json.dump(data, json_file, indent=4)
        print("Load is done!")
...
def load_fixtureTStatsJsonData(tmp_data, league_id):
    ...
    with open("%s/%s_Ftstats.json" % (directory, league_id), "w") as json_file: #
        json.dump(data, json_file, indent=4)
        print("Load is done!")
