...
for i in range(4):
#for i in range(276):
    now_date = (datetime.utcnow().date() - timedelta(days=i+10)).strftime("%Y-%m-%d")
    now_week = str((datetime.utcnow().date() - timedelta(days=i+10)).today().isocalendar()[1])
    now_date_local = (datetime.utcnow().date() - timedelta(days=i+10)).strftime("%y%m%d")
